...
export async function renderWip(ctx: PanelContext) {
  ...
  if (cs) {
    ...
    card2.innerHTML = `<b>Contractor statements</b> <span class="meta">(percentage-of-completion)</span>`
      ...
      + `<tr><td>Revenue earned (POC)</td><td class="num">${usd(inc.revenue_earned)}</td></tr>`
      + `<tr><td>Cost of revenue</td><td class="num">${usd(inc.cost_of_revenue)}</td></tr>`
      + `<tr class="fin-total"><td>Gross profit (${inc.gross_margin_pct}%)</td><td class="num">${usd(inc.gross_profit)}</td></tr>`
      + `<tr><td colspan="2" style="font-weight:700;padding-top:4px">Contract position (balance sheet)</td></tr>`
      + `<tr><td>Contract asset — under-billings</td><td class="num">${usd(pos.contract_asset_underbillings)}</td></tr>`
      + `<tr><td>Contract liability — over-billings</td><td class="num">${usd(pos.contract_liability_overbillings)}</td></tr>`
      + `<tr><td>Retainage receivable</td><td class="num">${usd(pos.retainage_receivable)}</td></tr>`
      + `<tr><td>Accounts payable (subs)</td><td class="num">${usd(pos.accounts_payable)}</td></tr>`
      + `<tr class="fin-total"><td>Net contract working capital</td><td class="num" style="color:${pos.net_contract_working_capital < 0 ? "var(--status-crit)" : "var(--status-good)"}">${usd(pos.net_contract_working_capital)}</td></tr>`
      + `</table>`;
    body.append(card2);
  }
