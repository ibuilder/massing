...
import { georeferencingButton, projectOriginButton, projectUnitsButton } from "./projectSetupPanel";
...
export function buildQaSection(d: QaDeps): void {
  ...
  const selectedGuid = () => d.selectedGuid();
        ...
        // MODEL-SETUP — the survey-basis READ moved to `projectSetupPanel.ts` to sit beside the
        // two repairs for the condition it diagnoses (units and origin), which is the same
        // argument `repairPanel.ts` makes for wall joins. Taking it along is also what PAYS
        // for the two new controls: this file is exactly on its size ratchet.
        const setupDeps = { api, pid, toolBtn2, out, container, notify, authorAndReload };
        b.appendChild(georeferencingButton(setupDeps));
        b.appendChild(projectUnitsButton(setupDeps));
        b.appendChild(projectOriginButton(setupDeps));
        ...
        b.appendChild(toolBtn2("📐 Alignment check (storey + origin)", () => withLoading(container, "Checking model alignment", () =>
          runAlignmentPanel({ pid, api, toast, showResult, resultNote, kvTable,
                              setOut: (t) => { out.textContent = t; } }))));
