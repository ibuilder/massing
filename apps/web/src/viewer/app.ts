...
export function initViewerApp(ctx: ViewerCtx): ViewerApp {
  ...
  async function buildToolsPanel() {
    ...
    if (glBody) {
      ...
      // R39-DECOMP-VIEWER ⑮ — the unified content + family library moved to
      // `tools/contentLibrarySection.ts`. Checked for ⑭'s shape before moving: this block assigns
      // to nothing declared outside it and installs no listener, so there is no cross-rebuild
      // state to strand. `lastPoint` still crosses as an ACCESSOR.
