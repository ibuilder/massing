...
11. **G-12 (S) Four more read-modify-writes on tables with no concurrency token** —
   `drawingset.revise_sheet`, `connections.put_mappings`, `proforma.sync_gmp_to_hard` and
   `proforma.sync_model_to_hard`. (`proforma.put_property` was one of the original six and is now
