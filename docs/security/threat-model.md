...
10. **G-11 — CLOSED 2026-09-14 (RMW-LOCKGAP). Six IFC-authoring routes derived a new model version outside the project lock** —
   ...
   open must still be open, so closing one cannot leave a stale gap recorded.
   ...
   **All six now hold `pid_lock.mutating(pid)` across the whole read → derive → apply →
   pointer-swap, and the lock is proved by behaviour rather than by its presence.**
   `services/api/test_authoring_lock.py` runs two concurrent bakes against a stubbed recipe and
   asserts the second reads the FIRST's output; neutering `pid_lock.mutating` makes both read the
   same base. *A test that greps for the lock would pass on a lock that spans only the assignment* —
   the defect review caught one level up on PR #552 — so what is asserted is the chaining, which is
   observable, not the lock, which is not.

   Three things the uniform fix got wrong and had to be corrected:

   - **Two of the six are `async def`.** `pid_lock.mutating` is synchronous and blocks on a Postgres
     advisory lock, so taking it inline in `import_families` / `content_import` would park the event
     loop behind somebody else's upload — *a rare lost model version traded for a routine
     server-wide stall*, which is the v0.3.703 SSE failure arriving by a different door. Both now run
     the locked section through `run_in_threadpool`.
   - **The first draft of `import_family_pack` left `db.commit()` outside the lock.** An uncommitted
     pointer swap is invisible to the next lock-holder, which would read the old version and derive
     from it — the same race moved rather than closed. *A lock proves something about the interval it
     spans, and the interval has to end after the write is durable.*
   - **`_restore_version` read `p.source_ifc` once more after the lock released**, to build its
     response. Harmless today (`expire_on_commit=False` keeps this session's own value) — and the
     sweep refused to certify it anyway, because a certifier that reasons about which stragglers are
     harmless is one bad reading away from blessing one. The value is now captured inside.

   The lock for undo/redo sits in the shared `_restore_version` helper rather than in `edit_undo` and
...
11. **G-12 (S) Five more read-modify-writes on tables with no concurrency token** —
   ...
   **closed** under the per-project lock — see the `dev_property` note in the concurrent-edit row
   ...
   the build. The five that remain are deliberately **not** ranked here. The first draft of this
