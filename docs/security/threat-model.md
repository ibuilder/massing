...
11. **G-12 (S) Five more read-modify-writes on tables with no concurrency token** —
   ...
   `connections.put_mappings`, `proforma.sync_gmp_to_hard` and
   `proforma.sync_model_to_hard`. (`proforma.put_property` was the sixth and is now **closed** —
   see the `dev_property` note in the concurrent-edit row above.) Each reads a column and writes back a value derived from it on a
