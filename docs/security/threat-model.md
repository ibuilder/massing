...
11. **G-12 (S) Six more read-modify-writes on tables with no concurrency token** —
   `bim.promote_markup` (the shape `modules.promote_comment` had before it became a conditional
   UPDATE, so a live instance of an already-solved defect), `drawingset.revise_sheet`,
   `connections.put_mappings`, `proforma.put_property`, `proforma.sync_gmp_to_hard` and
   `proforma.sync_model_to_hard`. Each reads a column and writes back a value derived from it on a
...
12. **G-10 (S) Two JSON collections still lose a concurrent write** — `Scenario.shared_with` (a
