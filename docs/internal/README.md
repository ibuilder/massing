...
| `plan-2026-08-10.md` | **LIVE, not superseded** — plan state after v0.3.915-924: the frontend-framework question CLOSED (neither React nor Reflex; the CRUD third is the server-side candidate, measured at ~13k of ~47k hand-written lines), sibling-integration state per repo, **CC0 settled 2026-08-19**, massingviser vs modelmaker still the user's, and the next four items in order. | not superseded — active |
...
| `tech-debt-2026-08-09.md` | **Audit.** Eight scored items; the top three tie at P=24 (two dependency bumps carrying CVEs in the production lock, plus the composer defect). Its "Checked and NOT counted" section is the load-bearing half — two candidate findings were dropped after verification, including one I had already half-written up. **Partial supersession:** as of v0.3.986, `pypdf==6.15.0` and `cryptography==50.0.0` are in the lock; `diskcache==5.6.3` remains. |
| `upgrade-plan-2026-08-19.md` | **Opinionated audit + decision brief at v0.3.986. Nothing built from it.** Whole-product read (architecture, performance, security, debug, UI/UX) so the operator can pick; not a work list. Live work stays in [../roadmap.md](../roadmap.md). |
| `massing-cloud-sso.md` | **Integration notes, 2026-08-24 — LIVE, describes shipped code.** massing.cloud as the OAuth2/PKCE identity broker plus the Vault project library: how to enable it, the three decisions a reviewer should question, and what is deliberately absent. Carries **one open dependency on the site**: `userinfo` publishes no role, verified against the plugin source, so the admin/editor mapping is built and inert until massing.cloud adds it — the four-line filter that closes it is in the document. Every claim it makes about the code is asserted in `services/api/test_massing_cloud_sso.py`, per rule 2 below. |

| `asset-rights-nft-design.md` | **Integration study + shipped-scope record, 2026-08-27.** Written before any code, against an incoming brief for NFT-backed `.mass` releases. Records **five mismatches** between that brief and this repository — two of which would have produced a duplicate or broken subsystem: a deterministic release hash already exists (R23-DIGEST), and `import_bundle` regenerates the project id, so provenance could not hang on it. Also notes that `ShareToken` already delivers revocable scoped entitlements without a chain, and that [../roadmap-completed.md](../roadmap-completed.md) carries a prior token-last decision (about *securities* tokens — adjacent, not identical). **Steps 1-3 built** (opt-in release manifest, a stable project asset id, Ed25519 signing); the chain half is unbuilt and gated on five decisions listed in the document. Claims about the code are asserted in `services/api/test_asset_rights.py`, per rule 2 below. | not superseded — active |
