# Runway for Claude Code — 2026-08-20, after v0.3.1027
...
`origin/main` is still **v0.3.986** (`a67c4732`). The Cursor stack is **v0.3.988–1027**, unmerged,
draft PRs, each stacked on the previous. Your job is to **land that stack onto main**, not to
re-implement it.

1. `git fetch origin`. Tip of the stack: `origin/cursor/scale-seam-precon-6e15` (**v0.3.1027**).
2. Land **in table order**, one PR at a time. After each merge, rebase the remainder onto the new
   `main`. **Keep the later version numbers** (do not rebump 1027 down to 987).
...
| 24 | [#317](https://github.com/ibuilder/massing/pull/317) | `cursor/scale-seam-drawings-6e15` | **1020** vs #316 |
| 25 | [#318](https://github.com/ibuilder/massing/pull/318) | `cursor/scale-seam-elements-6e15` | **1021** vs #317 |
...
| 30 | [#323](https://github.com/ibuilder/massing/pull/323) | `cursor/scale-seam-ai-6e15` | **1026** vs #322 |
| 31 | this | `cursor/scale-seam-precon-6e15` | **1027** vs #323 |
...
| 1021 | SCALE-SEAM ⑯ — `/elements` mixin; `client.ts` 3,482 → 3,412 |
| 1022 | SCALE-SEAM ⑰ — `/models` mixin; `client.ts` 3,412 → 3,353 |
...
| 1027 | SCALE-SEAM ㉒ — `/precon` mixin; `client.ts` 3,170 → 3,128 |
...
- Do **not** grow `apps/web/src/viewer/app.ts` or `apps/web/src/api/client.ts` without extracting
  first. Pins: `client.ts` **3,128**, `app.ts` **3,032**, `register.ts` **2,516**.
...
1. **SCALE-SEAM remainder** — next route-group by size (`/codecheck`, `/accounting`, …). ㉒ is `/precon`.
