# Runway for Claude Code — 2026-08-20, after v0.3.1021
...
`origin/main` is still **v0.3.986** (`a67c4732`). The Cursor stack is **v0.3.988–1021**, unmerged,
draft PRs, each stacked on the previous. Your job is to **land that stack onto main**, not to
re-implement it.

1. `git fetch origin`. Tip of the stack: `origin/cursor/scale-seam-elements-6e15` (**v0.3.1021**).
2. Land **in table order**, one PR at a time. After each merge, rebase the remainder onto the new
   `main`. **Keep the later version numbers** (do not rebump 1021 down to 987).
...
| 25 | this | `cursor/scale-seam-elements-6e15` | **1021** vs #317 |
...
- Do **not** grow `apps/web/src/viewer/app.ts` or `apps/web/src/api/client.ts` without extracting
  first. Pins: `client.ts` **3,412**, `app.ts` **3,032**, `register.ts` **2,516**.
...
1. **SCALE-SEAM remainder** — next route-group by size (`/models`, `/documents`, `/pdf`, `/mep`, `/topics`, …). ⑯ is `/elements`.
