...
reconciliations below. **Twenty of the twenty-one were already marked ✅ in the roadmap itself** — the
work was done, the entry said so, and nothing moved it. The phrase *"pending archive"* appears **17**
times in the roadmap they came from — **14 of them in lane-table cells**, which is the table an agent
reads to pick up work, and 3 in the item bullets themselves. One entry had been listed open for a
fortnight after landing.

**That is a different finding from the earlier reconciliations, and the difference is the useful
part.** Those closed items whose *premise* had gone stale — the entry described a gap that was
already filled, and nobody had re-read it. These did not go stale: every one was correctly marked by
whoever shipped it. The marking simply was not an action. **A status glyph is a note to a reader;
only a move is a change to the file** — and `roadmapLanes.test.ts` deliberately skips ✅ lines when it
builds the open population, so a shipped item that stays put is invisible to the one check that
reads this file. Nothing could go red, so nothing did.

The cost is concrete. `roadmap.md` was **3,715** lines carrying **30** pieces of open work; it is
**3,084** now, and **18 of the 21** archived codes were named in lane-table cells — the table an
agent reads to choose what to pick up. Anyone looking for work had to reject them first.

**Deliberately NOT moved: the three ⛔ items** (R43-CRUD-FRAGMENTS, R23-PICKING, R22-PHOTO-CV). A
closed-unbuilt or refused-in-place entry is a *decision*, not shipped work, and it earns its place
beside the open list precisely so nobody re-proposes it. R23-PICKING is also the live fixture
`roadmapLanes.test.ts` uses to prove it can SEE a ⛔ item before excluding it — archiving it would
have left that claim resting on a synthetic line, which is the drift the test says it exists to
prevent.

**What would stop the next one:** the ✅-skip is what makes an unarchived item cost nothing, so the
natural gate is a ceiling on how many ✅ entries may sit in `roadmap.md` at once — a ratchet, in the
same shape as that file's unowned-file ceiling. Not built here, and the reason is a real one rather
than a deferral: the right number is a judgement about how long a shipped entry may reasonably
linger before someone archives it, and picking it inside the pass that just took the count to zero
would set it from the least representative moment there is.

**The twenty-first was not marked at all.** A29-GUIDE-UNDERLAY ③ carried 🟡 and the annotation
*"(in flight, PR #199)"*. PR #199 merged on 2026-08-06 and the feature shipped in **v0.3.875** — so
that cell described a pull request as open for two hundred and fourteen releases. It is the one
entry here that the ✅-skip argument does *not* explain: this item was inside the open population the
whole time, advertised as available work in Lane E, and the reason nobody picked it up is probably
that "in flight" reads as *someone else has this*. **An in-progress marker is a claim with an expiry
date and no mechanism to notice it passing** — and unlike a ✅, it actively deters the reader who
would have discovered it was done.

The twenty-one, verbatim as they stood in `roadmap.md`:
...
* ✅ **A29-GUIDE-UNDERLAY ③** *(**SHIPPED v0.3.875**; PR #199 merged 2026-08-06 — and this entry said
  "in flight, PR #199" for **214 releases** after that. The PR is the most reliable staleness clock
  there is: it has a merge date, and nothing consulted it. Verified in the tree rather than from the
  PR title — `apps/web/src/viewer/guideUnderlay.ts` (18 KB) plus `guideUnderlay.test.ts` (12 KB), and
  `app.ts` imports it.)* — *trace over a plan.* A 2D reference image
...
### Band 1's own summary bullet, kept because deleting it broke a gate

The band-level ✅ bullet below is the two upload items' shared summary. Rewriting Band 1 in this pass
deleted it rather than moving it, and `test_env_documented` went red three minutes later: the phrase
*"`properties.py` rejects over `AEC_PROPS_MAX_MB` before parsing"* inside it was the **only** mention
of that flag anywhere the gate reads — `.env.example`, `README.md`, `docker-compose.yml`, `docs/**`.

**So the flag had never really been documented; a planning entry had been standing in for operator
documentation, and the gate could not tell the difference.** It is now in `.env.example` beside the
other caps, which is where an operator would look. Two lessons, and the second is the larger one:
*archiving is not deleting* — move the text, then decide what to cut — and **a gate satisfied by
prose in a roadmap is measuring the wrong document.**
...
## Archived 2026-08-17 — thirteen entries closed in v0.3.970–976

Moved out of `roadmap.md` because they carry ✅ and their work is shipped. **`RATCHET-SET` was deliberately left in the live roadmap** — its own text argues that the reasoning for why a scalar ceiling was the wrong instrument is worth reading before anyone adds another one, and that argument is only useful where people are still choosing instruments.

Five of these closed in one sitting (R45-SCHED-REACH, R45-SCHED-DEDUPE, R45-VENDOR-REACH, R40-EOT, R41-CLASH-TRIAGE) and the pattern across them is worth carrying forward: **three of R45's four "overlaps" were never overlaps** — the table had been built from names rather than capabilities. Premise-check before building, every time.
