...
**EMPTY again as of 2026-09-13.** Both entries that filled this band that day — SYSTEM-COLUMN-PHANTOM
and COURT-SPLIT, the second found *while closing the first* — are **CLOSED** below. COURT-SPLIT was
filed rather than folded in because it had a product decision attached; the decision was taken (the
server's "whose move is it" rule survives, with its premise now enforced) and it shipped.
...
- **COURT-SPLIT — the register screen and every server-side report disagree about whose court a
  record is in, on 10 states across 9 modules** — **✅ CLOSED 2026-09-13**, gated by
  `services/api/test_court_primary.py` *(S — Lane C)*
  ...
  **✅ CLOSED — and the enforcement the decision asked for could not be built as specified.** The
  call was "keep the server's rule and enforce its premise", the premise being that authors list the
  primary forward action first. Deriving "forward" from the declared state order is the obvious
  enforcement and is **measurably wrong**: `action_item.states` is `['done', 'open']`, alphabetical
  rather than a progression, and under that reading 45 states have no forward transition and 39 have
  several. *The premise is not checkable, so it is now DECLARED* — `"primary": true` on one
  transition, or the state listed as `resting`.

  **17 of 267 states need it**, because for the other 250 the choice cannot change the answer (199
  have one outgoing transition, 51 have several sharing one party set). Five answers moved: the two
  wrong primaries above, and three states that stop naming anyone —`decision.decided`,
  `information_container.published`, `spec_section.issued` were putting closed records into
  ball-in-court rollups. The register reads the value now instead of deriving it, from both routes,
  resolved through `workflow_config.effective` so a project's overridden workflow cannot reopen the
  same disagreement by another door.

  *One correction to this entry's own first draft: `coordination_issue.open` was listed above as a
  violation and is not one — `assign` is the owed move. It reached the list through the same
  state-order test that had to be discarded.*

  **Lane C owns it, alone.** The first draft of this entry said "Lanes B+C" and the Lane B row said
  "shared with Lane C" — which contradicts the invariant stated three paragraphs above the lane
  table: *every open item is assigned to exactly one*, and the stated reason is that the lanes own
  disjoint paths so two agents cannot collide on one item. `roadmapLanes.test.ts` could not catch
  it, because it checks where the CODE appears and the contradiction was in the prose beside it —
  the prose-versus-gate drift this file keeps re-learning, found by review. Lane C is the owner
  because the surviving definition has to be emitted from one place and the reports already consume
  the server's; `register.ts` dropping its local `ballInCourt` follows that call rather than being a
  second piece of work to pick up independently.
...
- ✅ ⭐ **DESKTOP-FROZEN — the shipped Linux app could not start at all** *(S — Lane C; **CLOSED**,
  ...
  **THE FIX FOR THIS FILE WAS NOT THE FIX.** Correcting `desktop.py` and stopping would have shipped
  an app that still could not start. Measured against `origin/main`: **16 sites across 12 files** —
  3 that raise, 12 that quietly leave the bundle, 1 that is fine. Two of the three raisers do it at
  IMPORT rather than inside a function, so the Linux app would have died one line later, in
  `services/api/src/aec_api/routers/authoring.py`, under a different filename.

  | site | index | max in a bundle | when |
  |---|---|---|---|
  | `services/api/src/aec_api/desktop.py` | `parents[4]` | 3 | in `web_dist()` |
  | `services/api/src/aec_api/plugin_registry.py` | `parents[4]` | 3 | in `_plugins_dir()` |
  | `services/api/src/aec_api/routers/authoring.py` | `parents[5]` | 4 | **module import** |

  **A crash found by running the app tells you where it stopped, not how many places are wrong.**
  The first traceback is a sample of the population. Every site now goes through
  `services/api/src/aec_api/apppaths.py`, which walks up to the repository's SHAPE instead of
  counting directories — so there is no index left to be off by one — and returns None in a bundle,
  where there is no checkout and a guessed path would name a temporary directory.

  **The quiet half was four times larger than the loud one.** Twelve sites were in range and still
  left the bundle, and nothing was ever red because every caller guards with `.exists()`:
  `services/api/src/aec_api/package.py` asked for
  `services/api/data/src` and `desktop.py`'s `modules_dir()` for `services/modules` — neither has
  ever existed, and both sat under comments naming the path they did not build.
  **A path expression that cannot fail loudly must not be written by hand.**

  *(These counts were "six sites in three files" and "nine more" in this entry's first draft,
  measured against a half-converted working tree instead of against `main`. Both were wrong in the
  direction that made the sweep look smaller than it was — the flattering direction, which is the
  one nothing ever contradicts. They now come from running the gate's own analyser over
  `git archive origin/main`, which is why they carry a ref.)*

  `services/api/test_frozen_paths.py` is the gate: it derives the bundled population from the
  `pathex` and `collect_*` calls in `services/api/desktop.spec` and `services/api/sidecar.spec` —
  never a list written here — and fails on any `parents[N]` that would leave the bundle. Its first
  draft used a hard-coded pair of roots, and a mutation dropping one of them **passed every
  assertion**, because that package has no offending site today; deriving from the specs closes it.
...
- **RMW-LOCKBOUND — a writer that waits for a project lock never gave up** *(M — **Lane C**; opened
  ...
  *Filed against Lane G in this entry's first draft, because that is where the 30 call sites are. The
  fix is entirely in `services/api/src/aec_api/pid_lock.py`, which is Lane C's — and the Lane G row
  below already records that exact lesson from a previous item: a lane assignment made from where a
  defect SHOWS is wrong whenever the cause is one layer down. Written down, read, and repeated in the
  same file.*
...
- ✅ **RMW-LOCKGAP — all twelve IFC-authoring routes now derive a model version under the
  per-project lock** *(M — Lane G; **CLOSED 2026-09-14**, PR #555 `8503b0be` · gap G-11)*
  ...
  read-modify-write — while their six siblings (`edit`, `edit_graph`, `edit_batch`, `macros_run`,
  `option_activate`, `mcp_tools._run_recipe`) did the identical thing **inside
  `pid_lock.mutating(pid)`**, one carrying the comment *"same RMW race as /edit — serialize per
  project"*. So the control existed, was named in the threat model, and covered half its population:
  two concurrent `place_family` calls lost one placement with both callers answered 200.

  All six are wrapped. The lock is proved by **behaviour** in `services/api/test_authoring_lock.py`
  — two concurrent bakes, the second must read the first's output — rather than by its presence,
  because *a test that greps for the lock would pass on a lock that spans only the assignment*.
  Three things the uniform fix got wrong and had to be corrected are recorded against G-11 in
  [`docs/security/threat-model.md`](security/threat-model.md); the shortest is that two of the six
  are `async def`, so taking a blocking advisory lock inline would have traded a rare lost model
  version for a routine server-wide stall — the v0.3.703 SSE failure by another door.

- ✅ **RMW-TOKEN — the two remaining JSON collections stopped losing a concurrent write, and not by
  the mechanism this entry prescribed** *(S — Lane G; **CLOSED**, `dev_property` 2026-09-13 in PR
  #552 `c7fdc658`, `shared_with` 2026-09-23 in PR #572 `cd5d9e24` · gap G-10)*

  `routers/proforma.share_scenario` appended to `Scenario.shared_with` and
  `routers/realestate.save_appraisal` merged into `Project.dev_property`. Both read a JSON collection
  and wrote back a value derived from it, so a concurrent second grant or second appraisal save was
  silently dropped — and each caller's response echoed its own target either way, so neither could
  tell. Both now hold `pid_lock.mutating(...)` across refresh, merge and commit.

  **Three things about how this closed are worth more than the fix.**

  *The prescription was wrong.* This entry said closing it "is a migration (add the column,
  backfill, route both writers through a CAS), not a copy of the existing fix". No column was added,
  nothing was backfilled and neither writer got a compare-and-swap. The reasoning that ruled out the
  CAS was sound — `scenarios` and `projects` carry no `modified_at`, and JSON equality is not a swap
  that behaves the same on SQLite and Postgres, *the identical trap `with_for_update()` is*. It then
  jumped to the most expensive remaining option without enumerating the middle one, which was
  already in the tree and already serving the sibling gap next door. *Ruling out one mechanism is
  not the same as having enumerated the alternatives, and an entry that names only the expensive
  survivor sets a price nobody re-checks.*

  *Half of it was already closed the day before it was filed.* `save_appraisal` took
  `pid_lock.mutating(pid)` in PR #552 on 2026-09-13 — that pull request's own message says "both
  writers of that blob now hold `pid_lock.mutating(pid)`" — and this entry was written on
  2026-09-14 naming it as open. The gap it was split out of was measured before that fix landed and
  the split copied the reading rather than re-measuring it. *A gap inherited from an earlier sweep
  carries that sweep's timestamp, not today's.*

  *PR #556 is titled "RMW-TOKEN" and closed a different gap.* It fixed the `connections` pair
  (G-12); citing it here would look right and be wrong. *A pull request named after a roadmap entry
  is evidence about intent, not about which entry moved.*

  The lock rather than a swap is also the better control for both, for the reason
...
- ✅ **PIN-OPEN-ROW — clicking a register pin in the 3D viewer now opens its record**
  ...
  **The behaviour was also already written down as done.** `apps/web/src/pins/pins.ts`'s `load`
  ...
  init, the defect the developer tab beside it carries a comment about; it now returns its promise. **And that
  fix created the round's only new defect**, found by the next review pass: making `init()`
  retryable turned every partial mutation on the way to a rejection into reachable state — it
  reassigns `this.root` to the content pane and registers a `window` listener before it awaits
  `renderHome()`, so a retry would have nested a second shell inside the first. *Making a failure
  recoverable makes every partial mutation on the way to it reachable.* Guarded, and held by
  `apps/web/src/portal/portalRetry.test.ts`, whose two arms both red when the guard is removed.
...
**It happened again on 2026-09-24, at four times the scale and in two documents.** RMW-LOCKGAP and
RMW-TOKEN above both read 🟡 **OPEN** while `services/api/test_rmw_sweep.py` printed *"0 named
open"*, and the threat model carried G-10 and G-12 the same way. Both are now ✅ with the closing
pull request named. **The difference from 2026-08-13 is that this one is gated**: the band's thesis
is no longer held as a reminder to re-check. `services/api/test_gap_records.py` derives the open
sites from the sweep's own ledgers and reds the build on a record that cites that gate, reads as
...
|---|---|---|
| **A · Shell & IA** | `apps/web/src/shell/`, `apps/web/src/account/`, `apps/web/src/portal/portal.ts`, `apps/web/src/portal/favourites.test.ts`, `apps/web/src/portal/homes/`, `main.ts`, `apps/web/src/portal/prefs.ts`, `apps/web/src/portal/prefs.test.ts`, `apps/web/src/portal/densityToggle.ts`, `apps/web/src/portal/safetyCard.ts`, `apps/web/src/portal/safetyCard.test.ts`, `apps/web/src/portal/registerEmpty.test.ts`, `apps/web/src/portal/portalRetry.test.ts` *(the loose portal files whose importers are A's — claimed 2026-09-11; see the derivation below the table. **Unbackticked on purpose**: every backtick in THIS cell is parsed as a path claim, so writing the directory name as a citation here claimed the whole of it and clashed with Lane B)* | REL-4 · R40-RIBBON ② · R43-CRUD-FRAGMENTS *(⛔ CLOSED UNBUILT — rescoped 2026-08-11 before any code)* |
...
- ✅ **R22-PIPELINE** *(M → S — premise-checked 2026-08-07; CLOSED 2026-09-09, the last item being the
  department axis, which turned out to be a grouping the firm declares rather than a field)* — **multi-site pipeline dashboard** above the project workspace. Acquisition
...
- ◧ ⭐ **SCALE-SEAM — `client.ts` is no longer a god-file, but the split is not finished.** *(②–⓾ plus seven unnumbered slices — (81)–(87) — have shipped, **⓺ onward in the v0.3.1143 follow-on** — no version bump, tag lag already at the `test_release_current` bound)*
  ...
  **(94) moved two known members of SCALE-SEAM ㊻'s question.** `verifyAsbuilt` and
  ...
  *This entry said "(94) finished SCALE-SEAM ㊻" until review, and the distinction against (93) is
  ...
  and `addMepFitting` by name — so taking both completes an explicit list, and "finished" is
  defensible there. **㊻ named nothing**, so there is no list to complete and no basis for claiming
  the question is now whole; only that these two answer it and were missed. Fifth instance of one
  defect, and **the second consecutive PR in which I corrected a SUBSET of a phrase's sites** — here
  `model.ts` and `surface.test.ts` but not this line, after #410's pin without its changelog
  sentence. The rule I wrote for that one (derive the complement) applies to the fix as much as to
  the claim: grep every site of the wording, then edit.*
  ...
  **⓽ took the AI-drafting banner apart** — and it is the clearest case yet for grouping by
  ...
  three `/draft/{kind}` doors plus their multipart helper went to `ai.ts`; `extractSheets`
  (`/extract/sheets`, and it creates DRAWING records) went to `drawingSheets.ts`;
  `bidLevelingDetail` went to `procurement.ts`, whose header has claimed "bid levelling"
  since ⑥ while the method sat under a banner about RFIs. A route-prefix or banner split
  would have put all six in `ai.ts`. (1,512 → 1,462.)
