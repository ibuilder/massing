...
PER_FILE = {
    ...
    "apps/web/src/viewer/tools/qaSection.ts": 1_274,   # MODEL-SETUP: the survey-basis READ -> viewer/tools/projectSetupPanel.ts (1_305 -> 1_274), which is what PAYS for two new controls on a file that was exactly on its pin. `rebase_origin` and `convert_length_unit` were both in authoring_matrix.UNREACHED - "a capability no user can reach", which that module calls a defect - while being runnable through POST /edit the whole time; the only thing missing was a control, and the only reason one could not be added was that nothing exposed the CURRENT unit, so "convert to millimetres" was a coin flip the user was asked to call. The georeferencing report went with them because it is the DIAGNOSIS for the condition rebase_origin repairs, which is the same argument repairPanel.ts makes for wall joins: a detector and its fix on opposite sides of the app each look complete alone. # WALL-JOINS: the two REPAIR controls -> tools/repairPanel.ts (1_334 -> 1_305). The ratchet is the reason there is a module at all: this file sat at its pin EXACTLY, with zero headroom, and the wall-joins tool is 80 lines, so the gate refused — the same refusal the alignment note below records. Extracting only the new tool would have cost 2 net lines (an import and a call) and still been over, so the cleanup tool came along, which is what PAYS for the addition rather than merely relocating it. The two belong together on the merits: they are the only controls in the app that REPAIR the model rather than report on it, and they are the two the roadmap's "diagnosis ships, the repair does not" list got wrong in opposite directions.   # v0.3.1134: blank import gap closed (1_335 -> 1_334) while geometry results now show sourced basis.   # alignment check -> tools/alignmentPanel.ts (1_348 -> 1_335). The ratchet is why: adding the R41 yaw fit inline pushed this file 17 lines over and the gate refused, so the panel was extracted instead and came back three lines.
    ...
    "apps/web/src/portal/register/register.ts": 2_452,   # TM-RATES: the assignee row + its reassign control -> portal/register/assigneeRow.ts (2_459 -> 2_452), paying for the T&M pricing control this item mounts. The ratchet is what forced the shape: this file is pinned at its exact size, so a new mount had to be BOUGHT by an extraction rather than granted as headroom, and the block it bought from is a genuine leaf (one field, the assign endpoint, a reload callback). It also fixed a latent stored-XSS: the row built `Assignee: <b>${r.assignee}</b>` with innerHTML over a user-supplied id, and the extracted version uses textContent. # UPLOAD-POISON: the offline attachment queue + the rule for when a refusal is permanent -> portal/register/uploadQueue.ts (2_467 -> 2_459). A genuine leaf: it moves files to the server and reports what happened, reaching back for exactly three things - the API, a status line and a pins-changed nudge - which now arrive as a narrow UploadHost port rather than the whole PanelContext, so the queue can be driven in a test without standing up a panel. The net is only 8 lines because delegating stubs stay behind: portal.ts calls hookOnline/flushUploads on the register and registerOwnership.test.ts names both, so moving the CALLERS would have been a second change for no gain. Lines left; the seam is what was bought. # COL-PAIR: the reference cell + its resolve bound + the id shape -> portal/register/refCell.ts (2_505 -> 2_467). The ratchet went red on the pair-aware cell and the remedy is the one this file states: extraction, never headroom. A genuine leaf — it builds one <td> from a value and a resolved map, and reaches back for exactly one thing (what a click opens), which is now a callback. It lands beside fieldPairs.ts because the two answer the same question from opposite ends: which field a column shows, and how a reference looks once chosen. # R22-ENTITLEMENT (5): the comment thread + composer + the new promote control -> portal/register/recordComments.ts (2_516 -> 2_505). The ratchet went red on the promote control and the remedy is the one this file states: extraction, never headroom. The block is a genuine leaf — it touches the record's comments, the API and a reload callback, and nothing else on the class — and it follows the directory's own convention (elementTies.ts, schemaStale.ts, tiedElements.ts are already extracted the same way).
}
